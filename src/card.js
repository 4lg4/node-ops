...
let on = (card = defaultCard) =>
  exec(`networksetup -setairportpower ${card.replace(/\s/g, "\\ ")} on`);

let off = (card = defaultCard) =>
  exec(`networksetup -setairportpower ${card.replace(/\s/g, "\\ ")} off`);
...
export async function setDns(dns, card = defaultCard) {
  return exec(`networksetup -setdnsservers ${card.replace(/\s/g, "\\ ")} ${dns}`);
}

export async function getDns(card = defaultCard) {
  return exec(
    `networksetup -getdnsservers ${card.replace(/\s/g, "\\ ")}`
  ).then(dns => {
    const dnsClean = dns.trim();
    return dnsClean.indexOf('There aren') !== -1 ? 'empty' : dnsClean;
  });
}

export async function isLocalHost(card = defaultCard) {
  return getDns(card.replace(/\s/g, "\\ ")).then(dns => dns.indexOf("127.0.0.1") > -1);
}
